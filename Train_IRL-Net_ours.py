...
import cv2
import numpy as np
os.environ['CUDA_VISIBLE_DEVICES'] = "0,1"  # 指定使用哪块GPU运行以下程序‘0’代表第一块，‘1’代表第二块
import torch
...
image_index = 4
datasets_name = 'xian'  # 数据集名称
num_classes = 7  # 分类数 huhhot:11 beijing:10 shanghai:10 nanjing:11 xian:7 xian_whole:12
...
test_loader = Data.DataLoader(dataset=test_data, batch_size=BATCH_SIZE * 4, shuffle=False, num_workers=0)
...
for epoch in range(1, EPOCH + 1):
    ...
    accracy = test_model(model, test_loader)
    if accracy > test_accracy:
        test_accracy = accracy
        best_epoch = epoch
        print("test_accracy:{}".format(test_accracy))
        torch.save(model, os.path.join(save_path, '{}_best_{}_baseline-IRL-Net.pkl'.format(datasets_name, epoch)))
    torch.save(model, os.path.join(save_path, '{}_IRL-Net.pkl'.format(epoch)))
